...
static PtFraction SearchTree (PtABPNode proot, PtFraction pelem)
{
    ...
    else if(FractionCompareTo(proot -> PtElem, pelem) > 0) return SearchTree(proot -> PtLeft, pelem);
...
void ABPInsert (PtABP ptree, PtFraction pelem)	/*  */
{
    ...
    PtABPNode Node = ptree -> Root, Prev = NULL;
    if(ptree -> Root == NULL){
...
PtFraction ABPMin (PtABP ptree)
{
    ...
    PtFraction result = ptree -> Root -> PtElem;
    PtABPNode node = NULL;
    if(ptree -> Root -> PtElem != NULL) node = ptree -> Root -> PtLeft;
...
PtFraction ABPMax (PtABP ptree)
{
    ...
    Error = OK;
    if(ptree->Root != NULL) return MaxTree(ptree->Root);
    return NULL;
}
...
static PtQueue ABPFillQueue (PtABP ptree)
{
    ...
    PtABPNode root = ptree->Root;
    PtQueue ReturnQueue;
    PtQueue LevelingQueue;

    if(((LevelingQueue = QueueCreate(sizeof(PtABPNode))) == NULL || (ReturnQueue = QueueCreate(sizeof(PtABPNode))) == NULL)){
        Error = NO_MEM;
        return NULL;
    }

    if(QueueEnqueue(LevelingQueue, &root) == NO_MEM){
        Error = NO_MEM;
        QueueDestroy(&LevelingQueue);
        return NULL;
    }

    while(!QueueIsEmpty(LevelingQueue)){
        QueueDequeue(LevelingQueue, &root);
        QueueEnqueue(ReturnQueue, &root);
        if(root->PtRight != NULL){
            if (QueueEnqueue(LevelingQueue, &root->PtRight) == NO_MEM) {
                Error = NO_MEM;
                QueueDestroy(&LevelingQueue);
                return NULL;
            }
        ...
        if(root->PtLeft != NULL){
            if(QueueEnqueue(LevelingQueue, &root->PtLeft) == NO_MEM){
                Error = NO_MEM;
                QueueDestroy(&LevelingQueue);
                return NULL;
            }
...
void ABPReunion (PtABP ptree1, PtABP ptree2)
{
    ...
    while(!QueueIsEmpty(queue)){
        ...
        ABPInsert(ptree1, FractionCopy(node -> PtElem));
    }
    Error = OK;
...
void ABPDifference (PtABP ptree1, PtABP ptree2)
{
    ...
    while(!QueueIsEmpty(queue)){
        QueueDequeue(queue, &node);
        ABPDelete(ptree1, FractionCopy(node -> PtElem));
    }
    Error = OK;
}
...
void ABPIntersection (PtABP ptree1, PtABP ptree2)
{
    ...
    while(!QueueIsEmpty(queue)){
        QueueDequeue(queue, &node);
        if(!ABPSearch(ptree2, node -> PtElem))
            ABPDelete(ptree1, FractionCopy(node -> PtElem));
    }
    Error = OK;
}
